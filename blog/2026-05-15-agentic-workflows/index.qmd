...
**Four languages.** The code chunks themselves accept four programming languages as first-class citizens: Python, R, Julia, and Observable. As a data scientist I lean on Python and R the most, and switching between them inside the same document is as simple as changing the chunk header. Each language runs against its own kernel, so results, plots, and tables flow back into the rendered output without any glue code on my side.
