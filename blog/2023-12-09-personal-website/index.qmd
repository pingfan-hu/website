...
3.  I also used a customized `scss` file called `theme.scss` and another css file called `styles.css`. Both `scss` and `css` files are used to define personal page layout settings. The YAML grammar looks like this (You can see the full YAML code in the `_quarto.yml` file):
