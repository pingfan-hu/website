...
image: "/blog/banners/2025-04-12-github-action.png"
date: "2025-04-12"
categories: [Tutorial]
echo: true
eval: false
...
Netlify has a [basic walk-through](https://www.netlify.com/blog/2016/09/29/a-step-by-step-guide-deploying-on-netlify/) of website deployment upon GitHub push, which should solve the problem in most cases. I also documented [my version of tutorial](https://pingfanhu.com/blog/2023-12-09-personal-website/). However, it requires to fully construct the website on user's local machine and push the built folder (usually the `_site/` folder) to the repository. To do this, I need to always fully rebuild all website pages locally anytime I make changes, and the `_site/` folder grows larger and larger over time.
...
::: {.callout-tip}
1. In "Install R dependencies" step, you may need to modify the list of packages according to your own needs.
2. The "Install TinyTeX" step is only necessary if you have PDF output in your website.
3. In "Render Quarto Project" step, I set an environment variable `SITE_PASSWORD` to pass to Quarto. You can remove it if you don't need it.
4. Conduct to LLM to modify the workflow file according to your own needs.
:::
