...
[Obsidian](https://obsidian.md/) is a note-taking app that stores everything as `.md` files with real-time rendering. A free account is sufficient enough to sync your vaults to iCloud. I pair it with Claude Code using a `cd-obsidian` quick command. It works for almost anything: knowledge base, quick notes, plans, drafts.
...
[\@tw93](https://tw93.fun) recently released a collection of skills called [Waza](https://github.com/tw93/waza), which includes eight skills: `/think`, `/ui`, `/hunt`, `/check`, `/write`, `/learn`, `/read`, and `/health`.
...
Back then, I wrote every line of code by hand and organized every directory myself. [Stack Overflow](https://stackoverflow.com) was the go-to when I got stuck. Managing everything alone meant thinking like a project manager: track progress, allocate files, plan ahead. This habit stayed with me ever since. Project management is still one of the more useful skills I have, even now that Claude handles most of the implementation.
...
Claude Code launched in 2025. The largest difference is not the model itself, and in fact, Claude Code uses the same models as the Claude app. The difference is that designating a working directory becomes a primary step, so your context becomes every file inside it. That makes conversations much more grounded than a blank chatbot session.

With this mindset, Anthropic kept adding capabilities onto Claude Code. Every extension, whether plugins, skills, MCPs, or hooks, follows the same logic: files live in either your working directory or your home folder, and Claude Code picks them up from there.
...
Throughout this path, I learned to actively plan my work, and build a workflow around it and boost up my efficiency together with LLM agents. However, the ultimate takeaway is that no matter how strong the LLMs are, it's always good to think on your own. Again, as said by professor Patrick Winston in his *How to Speak* lecture, "Your success in life will be determined largely by your ability to speak, your ability to write, and the quality of your ideas, in that order." Can you see it? It's **YOUR** ability to speak, to write, and the quality of **YOUR** ideas.

So my last takeaway is: Never let the LLMs take over "thinking", which is your responsibility. Cheers~