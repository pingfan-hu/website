...
The main code directory for an R package is `R/`. The `inst/` directory contaisn additional resources like CSS and JS files, as well as examples, templates, etc. The `pkgdown/` directory is used for building the package website with the `pkgdown` package. The `man/` directory is auto-generated during the building process, extracting the `roxygen2` documentation from the R scripts. The `tests/` directory contains the test scripts, and `vignettes/` contains long-form documentation.

## Version Control

Speaking of version control, **Git** is the de facto standard. Pro developers always emphasize that Git is different from GitHub, and will usually start from the `git` commands. This is beneficial for CLI lovers, but it doesn't hurt to have a GUI if you wanna have a quicker start.

So, yes, these 3 are different:

1. [Git](https://git-scm.com): The version control system, starting with the `git` command in your Terminal, whose common commands are like `git init`, `git add`, `git commit`, `git push`, etc.
2. [GitHub](https://github.com): A web-based platform for hosting Git repositories, owned by Microsoft. An anecdote of GitHub as the larget open-source platform is that GitHub itself is not open-source.
3. [GitHub Desktop](https://github.com/apps/desktop): A GUI application for managing Git repositories hosted on GitHub.

My take is that: The truth of all these 3 things are different is less important for you as a developer. I recommend starting with GitHub Desktop if you are new to version control, and then gradually learn the `git` commands in Terminal as you get more familiar with the workflow. For me, I've been doing RSE development for 3 years mainly using GitHub Desktop.

Start by using GitHub as a "manual" iCloud or OneDrive: it is an online backup of your code, which is one of the primary benefits of using GitHub. Every time you reach to a milestone, just commit and push your code to GitHub. You can always go back to previous versions if needed. Till now you mostly use the `main` branch.
...
I mainly use Claude Code but I heard Codex and Gemini are also good. Here I show Claude Code as an example. Vibe Coding is now much easier with these CLI tools integrated into IDEs. It's an assistant to your coding experience, so you can focus more on the high-level design and logic, leaving the coding details to AI, which what they are good at.
...
I'd like to have a `WORKFLOW.md` file in the root direcotry of every project I'm working on. For example, a Python project might have the following workflow:
...
LLMs tend to be verbose, so you can write down your requirements in your `WORKFLOW.md` file, something like: "If the function self-explains, don't leave comment."

**Fonts, Themes and Cosmetics**

These are less important but they improve your overall experience. Here are the resources I've been using and feel good with:
