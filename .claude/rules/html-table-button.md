...
All styles live in `styles/theme.scss` under `// ---- Link Table ----`. Color variants available: `lnk-purple`, `lnk-blue`, `lnk-green`, `lnk-orange`. Mobile stacking is handled there too. **Do not add `<style>` blocks or `<link>` tags to component files.**
