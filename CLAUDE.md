...
- **`styles/site.js`** — injected after body on every page via `_quarto.yml`. Responsibilities: dynamic footer year, scroll fade-in animations (via IntersectionObserver on `.quarto-post`, `.quarto-grid-item`, `.presentation-box`), language labels above code blocks, and copy-button UX cleanup.
