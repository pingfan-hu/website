...
- **`talks/talks.csv`** — data-driven talks listing. `talks.qmd` reads this CSV with R (`tidyverse`, `htmltools`) and generates HTML cards. Adding a new talk = adding a row to the CSV. Columns: `Date`, `Title`, `Event` (supports markdown links), `Location`, `Name`, `Type`. `Type` must be `"slide"` or `"poster"` — slides get three buttons (Slides, PDF, GitHub) linking to `https://pingfan-hu.github.io/{Name}/` and `/talks/{Name}.pdf`; posters get only a PDF button.
